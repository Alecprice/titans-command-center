...
(() => {
  ...
  function homeNow(){if(route()!=='home'||document.querySelector('.v14-now'))return;const hero=document.querySelector('.fan-hero');if(!hero)return;const focus=gameFocus(),g=focus.game,current=focus.state==='game-window',move=latestMove(),story=latestStory(),injuries=state.intel?.injuries||[];const box=document.createElement('section');box.className='v14-now';box.innerHTML=`<header><div><small>${seasonPhase()}</small><h2>What matters right now</h2></div><a href="#fan">What changed →</a></header><div class="v14-now-grid"><a href="#live" class="v14-now-main"><small>${current?'CURRENT MATCHUP':'NEXT GAME'}</small><strong>${esc(gameName(g))}</strong><span>${esc(gameTime(g))}</span><b>${esc(g?(current?'Game window open':`Starts in ${countdown(g)}`):'Schedule loading')}</b></a><a href="#media"><small>LISTEN / WATCH</small><strong>${esc(g?.network||'Broadcast guide')}</strong><span>Kickoff converted to your timezone + authorized providers</span></a><a href="#transactions"><small>ROSTER</small><strong>${esc(move?.type||'Latest move')}</strong><span>${esc(move?.description||'No new transaction loaded')}</span></a><a href="#fan"><small>TEAM STATUS</small><strong>${injuries.length?`${injuries.length} injury-report row${injuries.length===1?'':'s'}`:'Report awaiting update'}</strong><span>${injuries.length?'Open the simple injury view':'We do not assume an empty feed means no injuries'}</span></a>${story?`<a href="#feed"><small>TOP INTEL</small><strong>${esc(story.title||'Latest Titans story')}</strong><span>Open Intel Feed →</span></a>`:''}</div></section>`;hero.insertAdjacentElement('afterend',box)}

  function gameDayQuick(){if(route()!=='live'||document.querySelector('.v14-gameday-quick'))return;const target=app.querySelector('.page-head')||app.firstElementChild;if(!target)return;const focus=gameFocus(),g=focus.game,current=focus.state==='game-window',last=latestFinal();const el=document.createElement('section');el.className='v14-gameday-quick';el.innerHTML=`<div><small>QUICK READ</small><strong>${g?`${esc(gameName(g))} · ${esc(gameTime(g))}`:last?`Last result: ${esc(gameName(last))}`:'Titans Game Day'}</strong><span>${g?(current?'Kickoff has passed. Open Game Day for verified game status and Listen / Watch for legal broadcast options.':`Kickoff in ${esc(countdown(g))}. Open Listen / Watch for your local kickoff time and legal broadcast options.`):'Live and postgame details appear here when game data is available.'}</span></div><div><a class="button primary" href="#media">Listen / Watch</a><a class="button" href="#fan">Opponent + fan hub</a></div>`;target.insertAdjacentElement('afterend',el)}
