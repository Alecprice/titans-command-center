(() => {
  ...
  function ensurePasswordStyle(){
    if(document.querySelector('style[data-account-password-v172]'))return;
    const style=document.createElement('style');
    style.dataset.accountPasswordV172='';
  ...
  function enhancePassword(){
    ...
    if(!(input instanceof HTMLInputElement)||input.dataset.passwordV172==='true')return;
    const label=input.closest('label');
    if(!(label instanceof HTMLLabelElement))return;

    input.dataset.passwordV172='true';
