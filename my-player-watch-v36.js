(() => {
  ...
  function mountPlayer(){
    ...
    const isWatched=list.some(item=>(id&&item.id===id)||item.name.toLowerCase()===name.toLowerCase());
  ...
  function mountHome(){
    ...
    root.innerHTML=`<header><div><small>PLAYER WATCH</small><h2>Your Titans watchlist</h2></div><span>${list.length}/${MAX_WATCHED} tracked</span></header>${list.length?`<div class="v36-watch-grid">${list.map(item=>`<article class="v36-watch-card"><a href="${item.id?`#player?id=${encodeURIComponent(item.id)}`:'#roster'}"><strong>${esc(item.name)}</strong><span>Open Player Intelligence →</span></a><button class="v36-watch-remove" type="button" data-v36-remove data-v15-profile-save data-player-id="${esc(item.id)}" data-player-name="${esc(item.name)}" aria-label="Remove ${esc(item.name)} from watchlist">×</button></article>`).join('')}</div>`:'<div class="v36-watch-empty">Open any player from the roster and tap <strong>Watch player</strong>. Signed-in accounts carry this list across devices.</div>'}`;
  }
  ...
  document.addEventListener('click',event=>{
    ...
    const match=item=>(id&&item.id===id)||item.name.toLowerCase()===name.toLowerCase();
