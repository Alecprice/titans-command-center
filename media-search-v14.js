...
(() => {
  ...
  const KNOWN_RADIO_CALLSIGNS=new Set('WGFX WCLE WQAK WKFN WZDQ WKOM WANT WCOR WZNG WAKM WQMV WJJM WRJB WMSR WGOW WCRK WOKI WKXD WKIM WXSM WCDT WIKQ WWIC WJOX WQLT WZZN WJTW WCBL WPTQ WSON WKDZ WAIN'.split(' '));
  ...
  const callsignHandoff=value=>{
    ...
    if(!call)return'';
    const canonical=call.toUpperCase();
    if(!KNOWN_RADIO_CALLSIGNS.has(canonical))return'';
    const exact=source.toUpperCase()===canonical;
    const contextual=RADIO_CONTEXT.test(source)||RADIO_FREQUENCY.test(source);
    return exact||contextual?canonical:'';
  };
