...
function renderAuditedIntelligence(root,profile,preseason,site){
  ...
  layer.addEventListener('click',event=>{
    ...
    const button=event.target.closest?.('[data-v16-favorite]');if(button){const next=(()=>{try{return JSON.parse(localStorage.getItem(PLAYER_PROFILE_KEY)||'{}')}catch{return{}}})(),on=button.getAttribute('aria-pressed')==='true';next.favorite=on?'':p.name;try{localStorage.setItem(PLAYER_PROFILE_KEY,JSON.stringify(next))}catch{}button.setAttribute('aria-pressed',String(!on));button.textContent=on?'☆ Make favorite':'★ Favorite';}
  });
}
