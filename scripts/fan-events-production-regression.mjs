...
const REPORT_PATH='/tmp/fan-events-production-smoke.json';
const ALLOWED_PROVIDERS=new Set(['Ticketmaster','SeatGeek','Eventbrite','Skiddle']);
const SECRET_MARKERS=['EVENTBRITE_PRIVATE_TOKEN','EVENTBRITE_OAUTH_TOKEN','SKIDDLE_API_KEY','TICKETMASTER_API_KEY','SEATGEEK_CLIENT_ID','SEATGEEK_CLIENT_SECRET'];
...
try{
  ...
  const rawConfigured=payload?.configuredProviders&&typeof payload.configuredProviders==='object'?payload.configuredProviders:{};
  const configuredProviders=Object.fromEntries([...ALLOWED_PROVIDERS].map(name=>[name.toLowerCase(),Boolean(rawConfigured[name.toLowerCase()])]));
  const configuredCount=Object.values(rawConfigured).filter(Boolean).length;
  ...
  for(const result of providerResults){
    ...
    providerHealth[String(result.provider).toLowerCase()]={ok:result?.ok!==false,count:number(result?.count),displayedCount:number(result?.displayedCount)};
  }
  ...
  for(const marker of SECRET_MARKERS)assert(!serialized.includes(marker),`response leaked secret marker ${marker}`);
