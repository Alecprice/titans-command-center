...
def stabilize_mobile_sheet(driver):
    driver.execute_script("""
      if(!document.querySelector('style[data-mobile-nav-smoke-stable-sheet]')){
        ...
        style.textContent='@media(max-width:760px){#sidebar{transition:none!important;animation:none!important}}';document.head.appendChild(style);
      }
    """)
...
try:
    for width,height in [(390,844),(360,800)]:
        ...
        try:
            ...
            state=wait(d,"""const menu=document.querySelector('#menu-button'),dock=document.querySelector('.mobile-nav'),more=document.querySelector('#mobile-more-button'),game=document.querySelector('.mobile-game-action'),search=document.querySelector('#mobile-search-button');if(!dock||!more||!game||!search)return null;const mr=menu?.getBoundingClientRect(),dr=dock.getBoundingClientRect(),gr=game.querySelector('span').getBoundingClientRect();return {vw:innerWidth,vh:innerHeight,overflow:document.documentElement.scrollWidth>innerWidth+1,desktopMenu:menu?{x:mr.x,y:mr.y,w:mr.width,h:mr.height,display:getComputedStyle(menu).display}:null,dock:{x:dr.x,y:dr.y,w:dr.width,h:dr.height,display:getComputedStyle(dock).display},gameIcon:{y:gr.y,h:gr.height},targets:[...dock.querySelectorAll('a,button')].map(x=>({h:x.getBoundingClientRect().height,w:x.getBoundingClientRect().width,label:x.textContent.trim()})),active:[...dock.querySelectorAll('.active')].map(x=>x.textContent.trim())};""")
            if state['overflow']: raise RuntimeError(f'horizontal overflow at {width}: {state}')
            if state['desktopMenu'] and state['desktopMenu']['display']!='none': raise RuntimeError(f'desktop menu should be hidden when mobile dock owns navigation at {width}: {state}')
            ...
            if not any(x['label']=='Game' for x in state['targets']) or not any(x['label']=='Search' for x in state['targets']) or not any(x['label']=='More' for x in state['targets']): raise RuntimeError(f'primary dock actions missing at {width}: {state}')
