...
def assert_truth(state,label):
    ...
    for phrase in (
        'Starting prices are not substituted for checkout.',
        'Starting prices, unentered fees, seat quality, and projected spending are excluded.'
    ):
        if phrase not in cost:raise RuntimeError(f'{label}: Actual Cost Compare lost truth copy {phrase!r}')
