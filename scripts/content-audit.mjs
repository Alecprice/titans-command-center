...
check('fallback roster reflects the resolved Aug. 21 official roster moves',()=>{assert.equal(team.rosterCoverage.fallbackType,'cross-source-audited-snapshot');assert.equal(team.rosterCoverage.fallbackPlayers,roster.length);assert.equal(team.rosterCoverage.asOf,'2026-08-22');assert.equal(team.rosterCoverage.sourceConflict,'');assert.equal(roster.length,95);assert.equal(roster.filter(p=>p.status==='Active').length,91);assert.equal(roster.filter(p=>p.status==='Reserve/Injured').length,4);assert.ok(roster.some(p=>p.name==='Milo Eifler'&&p.number==='45'&&p.status==='Active'));assert.ok(roster.some(p=>p.name==='Tanoh Kpassagnon'&&p.number==='58'&&p.status==='Active'));assert.ok(roster.some(p=>p.name==='Nazeeh Johnson'&&p.status==='Reserve/Injured'));assert.equal(roster.some(p=>p.name==='Matt Lauter'),false);assert.equal(roster.some(p=>p.name==='Sean Brown'),false);assert.equal(roster.some(p=>p.name==='Dominic Richardson'),false);});
...
check('fallback feed carries the current Aug. 21 transaction',()=>{const item=feed.find(x=>/Tanoh Kpassagnon/i.test(x.title));assert.ok(item);assert.match(item.url,/tennesseetitans\.com\/news\/titans-sign-de-tanoh-kpassagnon/);assert.match(item.summary,/Milo Eifler/i);assert.match(item.summary,/Matt Lauter/i);assert.match(item.summary,/Nazeeh Johnson/i);});
