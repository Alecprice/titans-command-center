...
check('fallback roster reflects the Aug. 24 Carrico and Eifler move',()=>{assert.equal(team.rosterCoverage.fallbackType,'cross-source-audited-snapshot');assert.equal(team.rosterCoverage.fallbackPlayers,roster.length);assert.equal(team.rosterCoverage.asOf,'2026-08-24');assert.equal(team.rosterCoverage.sourceConflict,'');assert.equal(roster.length,96);assert.equal(roster.filter(p=>p.status==='Active').length,91);assert.equal(roster.filter(p=>p.status==='Reserve/Injured').length,5);assert.ok(roster.some(p=>p.name==='Reid Carrico'&&p.status==='Active'));assert.ok(roster.some(p=>p.name==='Milo Eifler'&&p.number==='45'&&p.status==='Reserve/Injured'));assert.ok(roster.some(p=>p.name==='Tanoh Kpassagnon'&&p.number==='58'&&p.status==='Active'));assert.ok(roster.some(p=>p.name==='Nazeeh Johnson'&&p.status==='Reserve/Injured'));assert.equal(roster.some(p=>p.name==='Matt Lauter'),false);assert.equal(roster.some(p=>p.name==='Sean Brown'),false);assert.equal(roster.some(p=>p.name==='Dominic Richardson'),false);});
check('remaining cross-source roster conflicts are explicit and fact-specific',()=>{assert.equal(auditedTeamContext.auditedOn,'2026-08-22');assert.equal(auditedTeamContext.knownConflicts.some(x=>/Matt Lauter/i.test(x.topic)),false);const brown=auditedTeamContext.knownConflicts.find(x=>/Sean Brown/i.test(x.topic));assert.ok(brown,'Sean Brown source conflict is missing');assert.match(brown.resolution,/exclude Sean Brown/i);assert.match(auditedTeamContext.sourcePolicy.rule,/fact-specific/i);});
...
check('fallback feed carries current official roster transactions',()=>{const current=feed.find(x=>/Reid Carrico/i.test(x.title));assert.ok(current);assert.match(current.url,/tennesseetitans\.com\/news\/titans-sign-lb-reid-carrico/);assert.match(current.summary,/Milo Eifler/i);assert.match(current.summary,/Injured Reserve/i);const prior=feed.find(x=>/Tanoh Kpassagnon/i.test(x.title));assert.ok(prior);assert.match(prior.summary,/Matt Lauter/i);assert.match(prior.summary,/Nazeeh Johnson/i);});
