...
check('fallback roster is the full dated audited snapshot',()=>{assert.equal(team.rosterCoverage.fallbackType,'full-audited-snapshot');assert.equal(team.rosterCoverage.fallbackPlayers,roster.length);assert.equal(roster.length,95);assert.equal(roster.filter(p=>p.status==='Active').length,91);assert.equal(roster.filter(p=>p.status==='Reserve/Injured').length,4);});
