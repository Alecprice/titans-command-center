(() => {
  ...
  function stationAffiliates(){return [
    ['WGFX','104.5 FM','Nashville flagship'],['WAKM','950 AM','Franklin'],['WANT','98.9 FM','Lebanon'],['WCOR','1490 AM','Lebanon'],['WZNG','1400 AM','Shelbyville'],['WQMV','1060 AM','Waverly'],['WMSR','1320 AM','Manchester'],['WGOW','1150 AM','Chattanooga'],['WXSM','640 AM','Tri-Cities'],['WCDT','1340 AM','Winchester']
  ]}
  ...
  function watchSection(g){const providers=providerCards(g);return `<section class="media-watch"><header><div><small>WATCH</small><h2>${g?`How to watch ${esc(gameLabel(g))}`:'How to watch the Titans'}</h2><p>${g?`${fmt(g.date)} · ${esc(g.network||'Network TBD')}`:'Authorized viewing options are matched from the game's network and your market.'}</p></div><a class="button" href="${OFFICIAL.titansWatch}" target="_blank" rel="noopener noreferrer">Official Titans guide ↗</a></header><div class="media-watch-grid">${providers.map(p=>`<a class="media-watch-card" href="${safeUrl(p.url)}" target="_blank" rel="noopener noreferrer"><small>${esc(p.badge)}</small><strong>${esc(p.name)}</strong><span>${esc(p.note)}</span><b>Open provider ↗</b></a>`).join('')}</div><div class="media-watch-note"><strong>Why we route instead of embedding live games</strong><p>NFL live video is protected by broadcaster authentication, DRM, market rules and commercial media rights. There is no public third-party API that legally hands this site the raw game stream. This page sends fans to the authorized service and can become more precise as we add licensed broadcast/TV-listing data.</p></div></section>`}
  ...
  function homeCard(){if(route()!=='home'||document.querySelector('.media-home-card'))return;const hero=document.querySelector('.fan-hero');if(!hero)return;const g=nextGame(),card=document.createElement('section');card.className='media-home-card';card.innerHTML=`<div><small>LISTEN / WATCH</small><strong>${esc(g?gameLabel(g):'Titans media')}</strong><span>${g?`${fmt(g.date)} · ${esc(g.network||'Network TBD')}`:'Find the authorized radio or streaming option.'}</span></div><a href="#media">Open media center →</a>`;hero.insertAdjacentElement('afterend',card)}
