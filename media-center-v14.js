(() => {
  ...
  function areaSwitch(){return `<div class="media-area-switch" role="group" aria-label="Media area"><button type="button" data-media-area="nashville" class="${state.area==='nashville'?'active':''}" aria-pressed="${state.area==='nashville'}">Nashville home market</button><button type="button" data-media-area="us" class="${state.area==='us'?'active':''}" aria-pressed="${state.area==='us'}">Elsewhere in U.S.</button><button type="button" data-media-area="international" class="${state.area==='international'?'active':''}" aria-pressed="${state.area==='international'}">International</button></div>`}
  ...
  function radioSection(g){
    ...
    return `<section class="media-radio-deck"><div class="media-radio-top"><div><small>LISTEN</small><h2>Titans Radio</h2><p>${intro}</p></div><div class="media-radio-dial"><span>104.5</span><small>THE ZONE</small></div></div>${localDeck}${radioProviderGrid()}<details class="media-affiliates"><summary>Selected terrestrial Titans Radio affiliates</summary><div>${stationAffiliates().map(x=>`<div><strong>${x[0]}</strong><span>${x[1]}</span><small>${x[2]}</small></div>`).join('')}</div><p>This is a selected regional subset. <a href="${OFFICIAL.titansAffiliates}" target="_blank" rel="noopener noreferrer">View the complete official 2026 affiliate list ↗</a></p><p>Use your actual radio for AM/FM reception. iPhones do not expose an AM/FM tuner to websites, so web playback uses authorized digital sources instead.</p></details></section>`
  }
