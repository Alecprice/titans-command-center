...
(() => {
  ...
  function playerTrends(){
    ...
    return [...grouped.values()].slice(0,3).map(list=>{const p=list[0],points=list.slice(0,5).map(r=>({week:r.week,value:statValue(r.stats,['passing_yards','pass_yds','rushing_yards','rush_yds','receiving_yards','rec_yds','total_tackles','tackles','sacks'])})).filter(point=>point.value!=null);const max=Math.max(...points.map(point=>point.value),1);return `<div class="v13-trend"><strong>${esc(p.name)}</strong><span>${esc(p.position)}</span>${points.length?`<div class="v13-bars">${points.map(point=>`<i style="--h:${Math.max(8,Math.round(point.value/max*100))}%" title="${point.value}"><b>${point.value}</b><small>${esc(point.week??'')}</small></i>`).join('')}</div>`:'<p>Loaded stats do not yet include a simple trend metric for this position.</p>'}</div>`}).join('');
  }
