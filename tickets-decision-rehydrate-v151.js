(() => {
  ...
  if(window.__TitansTicketDecisionRehydrateV151)return;
  ...
  const state=window.__TitansTicketDecisionRehydrateV151={checks:0,repairs:0,last:null};
  ...
  function wakeDecisionOwners(center,items){
    ...
    center.dispatchEvent(new CustomEvent(SHORTLIST_CHANGE,{bubbles:true,detail:{count:items.length,keys:[...keys],reason:'rehydrate-v151'}}));
  ...
  function reconcile(allowRetry=true){
    frameQueued=false;
    retryQueued=false;
    ...
    if(!report.commandReady&&allowRetry){
      retryQueued=true;
      requestAnimationFrame(()=>reconcile(false));
      return;
    }

    if(!report.commandReady)return;
    state.repairs+=1;
    wakeDecisionOwners(center,items);
  }

  function schedule(){
    if(frameQueued||retryQueued)return;
    frameQueued=true;
    requestAnimationFrame(()=>reconcile(true));
  }
