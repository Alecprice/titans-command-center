...
(() => {
  ...
  const LAST_GOOD_KEY='titans:tickets-last-good-v53';
  const CACHE_TTL=5*60*1000;
  const LAST_GOOD_TTL=24*60*60*1000;
  const state={payload:null,data:null,filter:'all',loading:null,dataLoading:null,renderToken:0,prefetched:false,refreshing:false,clientWarning:''};
  ...
  const priceValue=value=>{const number=finite(value);return number!=null&&number>0?number:null;};
  const money=value=>priceValue(value)!=null?new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}).format(Number(value)):'Price unavailable';
  ...
  function sortedGames(games){
    return [...(Array.isArray(games)?games:[])].filter(game=>state.filter==='all'||game.homeAway===state.filter).sort((a,b)=>(priceValue(a.lowestPrice)??Number.MAX_SAFE_INTEGER)-(priceValue(b.lowestPrice)??Number.MAX_SAFE_INTEGER)||String(a.datetimeUtc||a.datetimeLocal||'').localeCompare(String(b.datetimeUtc||b.datetimeLocal||'')));
  }
  function offerRow(offer,bestPrice){
    const price=priceValue(offer.lowestPrice);
    const delta=price!=null&&bestPrice!=null?price-bestPrice:null;
    const deltaPct=delta!=null&&bestPrice>0?Math.round((delta/bestPrice)*100):null;
    const count=finite(offer.listingCount);
    const deltaLabel=delta===0?'BEST PRICE':delta!=null?`+${money(delta)}${deltaPct>0?` · ${deltaPct}%`:''}`:'CHECK LIVE';
    return `<div class="tickets-offer-row${delta===0?' best':''}">
      ...
      <div class="tickets-offer-price"><b>${esc(money(price))}</b><span>${esc(deltaLabel)}</span></div>
  ...
  function comparisonGameCard(game,index){
    const offers=[...(Array.isArray(game.offers)?game.offers:[])].sort((a,b)=>(priceValue(a.lowestPrice)??Number.MAX_SAFE_INTEGER)-(priceValue(b.lowestPrice)??Number.MAX_SAFE_INTEGER));
    const best=priceValue(game.lowestPrice);
    const spread=priceValue(game.priceSpread);
    const spreadPct=finite(game.priceSpreadPct);
    const where=[game.venue?.name,game.venue?.city,game.venue?.state].filter(Boolean).join(' · ');
    const insight=spread!=null?`Next best is ${money(spread)} more${spreadPct>0?` (${spreadPct}%)`:''}`:offers.length>1?'Only one connected source returned a usable price':'Single-source price';
    return `<article class="tickets-compare-card${index===0?' global-cheapest':''}">
      <div class="tickets-price-block"><small>${index===0?'CHEAPEST TITANS TICKET NOW':'CHEAPEST NOW'}</small><strong>${esc(money(best))}</strong><span>${game.cheapestProvider?`via ${esc(game.cheapestProvider)}`:'starting price'}</span><em>${esc(insight)}</em></div>
      <div class="tickets-event-copy">
        <div class="tickets-event-tags"><span>${esc(game.homeAway==='home'?'HOME':game.homeAway==='away'?'AWAY':'TITANS')}</span><b>${offers.length} ${offers.length===1?'SOURCE':'SOURCES'} WITH OFFERS</b></div>
  ...
  function unavailableBody(){
    ...
    return `<section class="tickets-offline-state" role="status"><div><small>LIVE PRICE COMPARISON</small><h2>${state.payload?.configured?'Price sources unavailable':'Connect free price sources'}</h2><p>${esc(state.payload?.message||'Ticketmaster can remain live while SeatGeek and StubHub credentials are pending. Official game links stay available either way.')}</p></div><button type="button" class="tickets-primary-link" data-ticket-refresh>${state.refreshing?'Checking…':'Check prices now'}</button></section>${games.length?`<section class="tickets-upcoming"><header><small>UPCOMING TITANS GAMES</small><h2>Official game links remain available while price feeds are offline.</h2></header><div class="tickets-upcoming-list">${games.map(game=>`<a href="${OFFICIAL_URL}" target="_blank" rel="noopener noreferrer"><span><b>${esc(game.homeAway==='home'?'VS':'AT')}</b><strong>${esc(game.opponent||game.opponentAbbr||'Opponent TBD')}</strong></span><em>${esc(runtime.formatTeamKickoff?.(game.date,{weekday:'short',month:'short',day:'numeric'})||'Date TBD')}</em><i>View tickets ↗</i></a>`).join('')}</div></section>`:''}`;
  }
  function providerSummary(){
    const configured=Number(state.payload?.providersConfigured)||0;
    const successful=Number(state.payload?.providersCompared)||0;
    const failed=Number(state.payload?.providerFailures)||0;
    const priceCount=(Array.isArray(state.payload?.providerResults)?state.payload.providerResults:[]).filter(result=>result.ok).reduce((sum,result)=>sum+(Number(result.priceCount)||0),0);
    const stale=state.payload?.stale?' · showing last good':'';
    return `${successful}/${configured||successful} source${(configured||successful)===1?'':'s'} live${failed?` · ${failed} unavailable`:''} · ${priceCount} priced events · updated ${ago(state.payload?.fetchedAt)}${stale}`;
  }
  function providerStatusPanel(){
    const catalog=Array.isArray(state.payload?.providerCatalog)?state.payload.providerCatalog:[];
    if(!catalog.length)return'';
    const results=Array.isArray(state.payload?.providerResults)?state.payload.providerResults:[];
    const configured=state.payload?.configuredProviders||{};
    return `<section class="tickets-provider-health" aria-label="Ticket provider status"><header><strong>Marketplace status</strong><span>No API secrets are exposed here.</span></header><div>${catalog.map(item=>{
      const result=results.find(entry=>entry.provider===item.provider);
      const isConfigured=Boolean(configured[item.key]);
      const status=!isConfigured?'pending':result?.ok?'live':'down';
      const label=status==='live'?'Live':status==='down'?'Unavailable':'Not connected';
      const detail=status==='live'?`${Number(result?.count)||0} events · ${Number(result?.durationMs)||0}ms`:status==='down'?String(result?.errorCode||'temporary failure').replace(/-/g,' '):item.freeAccess;
      return `<span class="tickets-provider-pill ${status}"><b>${esc(item.provider)}</b><em>${esc(label)}</em><small>${esc(detail)}</small></span>`;
    }).join('')}</div></section>`;
  }
  function staleNotice(){
    if(!state.payload?.stale&&!state.clientWarning)return'';
    const stamp=state.payload?.lastGoodAt||state.payload?.fetchedAt;
    const reason=state.clientWarning||state.payload?.message||'Live providers are temporarily unavailable.';
    return `<div class="tickets-stale-banner" role="status"><strong>Using last known good ticket prices</strong><span>${esc(reason)}${stamp?` Last verified ${esc(ago(stamp))}.`:''} Open the marketplace before buying to confirm current inventory and checkout total.</span></div>`;
  }
  function shell(body){
    const live=Array.isArray(state.payload?.games)&&state.payload.games.length;
    const leader=live?sortedGames(state.payload.games)[0]:null;
    return `<section class="tickets-center-v47" data-ticket-center>
      <header class="tickets-hero"><div class="tickets-hero-copy"><div class="eyebrow">TICKETS · CHEAPEST CURRENT PRICE</div><h1>Titans Ticket Finder</h1><p>Compare every connected free marketplace feed at once. Games are sorted by the lowest current starting price we can verify, then you can jump straight to the marketplace showing that price.</p></div><div class="tickets-hero-price-rule"><small>${live?'CURRENT LEADER':'DEFAULT SORT'}</small><strong>${live?money(leader?.lowestPrice):'Cheapest first. Always.'}</strong><span>${live?`Lowest reported starting price · ${esc(leader?.cheapestProvider||'marketplace')}`:'No deal-score reshuffling.'}</span></div></header>
      <div class="tickets-trust-strip"><span><b>Comparison rule:</b> every configured free source is checked in parallel; one provider failing does not take the Ticket Center down.</span><span><b>Checkout rule:</b> starting prices are not guaranteed final totals. Compare fees, quantity and seat quality before buying.</span></div>
      ${staleNotice()}
      ${providerStatusPanel()}
  ...
  function readCache(key,maxAge){const cached=runtime.storage.getJSON(key,null);if(!cached||!cached.payload||!Number.isFinite(Number(cached.savedAt)))return null;const age=Date.now()-Number(cached.savedAt);return age<=maxAge?{...cached,age}:null;}
  function readCachedPayload(){
    const fast=readCache(CACHE_KEY,CACHE_TTL);
    if(fast)return fast.payload;
    const previous=readCache(LAST_GOOD_KEY,LAST_GOOD_TTL);
    if(!previous)return null;
    return {...previous.payload,stale:true,staleReason:'browser-last-known-good',lastGoodAt:new Date(Number(previous.savedAt)).toISOString(),staleAgeMs:previous.age,message:'Refreshing live ticket providers. Showing the last known good comparison until a current response arrives.'};
  }
  function cachePayload(payload){
    if(!Array.isArray(payload?.games)||!payload.games.length)return;
    const savedAt=Date.now();
    runtime.storage.setJSON(CACHE_KEY,{savedAt,payload});
    if(!payload.stale)runtime.storage.setJSON(LAST_GOOD_KEY,{savedAt,payload});
  }
  ...
  async function loadTickets(force=false,bypassEdge=false){
    ...
    state.loading=runtime.apiJson(endpoint,{ttl:bypassEdge?0:300000,force}).then(payload=>{if(token!==state.renderToken)return payload;if(payload){state.payload=payload;state.clientWarning='';}cachePayload(payload);state.refreshing=false;renderCurrent();if(!payload?.games?.length)loadFallbackData();return payload;}).catch(error=>{
      state.refreshing=false;
      const cached=readCache(LAST_GOOD_KEY,LAST_GOOD_TTL);
      if(cached){state.payload={...cached.payload,stale:true,staleReason:'browser-network-failure',lastGoodAt:new Date(Number(cached.savedAt)).toISOString(),staleAgeMs:cached.age};state.clientWarning='The live ticket request failed, so the most recent verified comparison is still shown.';}
      else state.clientWarning='The live ticket request failed. Official game links remain available.';
      renderCurrent();
      if(!state.payload?.games?.length)loadFallbackData();
      return null;
