...
const thDate=value=>{if(!value)return'TBD';const date=new Date(value);return Number.isNaN(date.getTime())?'TBD':new Intl.DateTimeFormat('en-US',{month:'short',day:'numeric',year:'numeric',timeZone:'UTC'}).format(date)};
const thStamp=value=>{if(!value)return'Update time unavailable';const date=new Date(value);return Number.isNaN(date.getTime())?'Update time unavailable':new Intl.DateTimeFormat('en-US',{month:'short',day:'numeric',hour:'numeric',minute:'2-digit',timeZone:'America/Chicago',timeZoneName:'short'}).format(date)};
...
const thSortTransactions=rows=>{
  const list=Array.isArray(rows)?rows:[];
  if(typeof window.TitansRuntime?.sortTransactionsLatestFirst==='function')return window.TitansRuntime.sortTransactionsLatestFirst(list);
  return list.map((transaction,index)=>{const raw=String(transaction?.date||'').trim(),time=raw?new Date(raw).getTime():Number.NaN;return {transaction,index,time}}).sort((a,b)=>{const aDated=Number.isFinite(a.time),bDated=Number.isFinite(b.time);if(aDated&&bDated)return b.time-a.time||a.index-b.index;if(aDated)return-1;if(bDated)return 1;return a.index-b.index}).map(row=>row.transaction);
};
