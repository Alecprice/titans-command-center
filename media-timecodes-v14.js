(() => {
  ...
  function listenLine(area){if(area==='nashville')return'Nashville radio → WGFX 104.5 FM The Zone; digital playback may have game/geo restrictions';if(area==='international')return'International rights vary → use the NFL by-country guide / Game Pass provider; official Titans audio may be geographically restricted';return'All-game audio → NFL+; Titans call also available through licensed services such as TuneIn Premium and SiriusXM where supported'}
  ...
  function renderGuide(){if(route()!=='media')return;const watch=document.querySelector('.media-watch');if(!watch||watch.querySelector('.media-tune-guide'))return;const g=nextGame(),area=areaMode();if(!g)return;const localZone=deviceZone(),local=zoneParts(g.date,localZone),et=zoneParts(g.date,'America/New_York'),ct=zoneParts(g.date,'America/Chicago'),utc=zoneParts(g.date,'UTC');const listenTitle=area==='nashville'?'104.5 FM · WGFX':area==='international'?'International options':'Titans live audio';const guide=document.createElement('section');guide.className='media-tune-guide';guide.innerHTML=`<header><div><small>NEXT TITANS BROADCAST</small><h3>${esc(gameName(g))}</h3><p>Everything you need to tune in, with kickoff converted automatically.</p></div><div class="media-countdown"><small>STARTS IN</small><strong data-media-countdown>${esc(countdown(g.date))}</strong></div></header><div class="media-time-grid">${timeRow('YOUR TIME',localZone,local,true)}${timeRow('EASTERN TIME','ET · New York',et)}${timeRow('NASHVILLE TIME','CT · Tennessee',ct)}${timeRow('WORLD TIME','UTC',utc)}</div><div class="media-tune-simple"><div><small>WATCH</small><strong>${esc(g.network||'Network TBD')}</strong><span>${esc(watchLine(g,area))}</span></div><div><small>LISTEN</small><strong>${esc(listenTitle)}</strong><span>${esc(listenLine(area))}</span></div></div><p class="media-time-help">Times use your device timezone automatically. ET/CT labels stay familiar while the displayed clock shows the correct daylight code (EDT/CDT) or standard code (EST/CST) for the game date.</p>`;watch.querySelector('header')?.insertAdjacentElement('afterend',guide);clearInterval(timer);timer=setInterval(()=>{const el=document.querySelector('[data-media-countdown]');if(el)el.textContent=countdown(g.date)},30000)}
  async function run(){await load();renderGuide()}
  ...
  if(app)new MutationObserver(()=>queueMicrotask(renderGuide)).observe(app,{childList:true,subtree:true});
  setTimeout(run,120);
})();
