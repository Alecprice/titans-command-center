(() => {
  ...
  const nextGame=()=>{const now=Date.now();return (data?.games||[]).filter(g=>{const t=gameTime(g);return Number.isFinite(t)&&t>now&&!/final|bye/i.test(String(g.status||''))}).sort((a,b)=>gameTime(a)-gameTime(b))[0]||null};
