...
const browser=await readFile(new URL('../scripts/ticket-budget-browser-smoke.py',import.meta.url),'utf8');
...
test('gate verifies the real Cloudflare deploy step and checks out the exact deployed SHA',()=>{
  ...
  assert.match(workflow,/python scripts\/ticket-budget-browser-smoke\.py/);
});
...
test('browser gate locks truth, edit/clear lifecycle, mobile geometry, and cleanup',()=>{
