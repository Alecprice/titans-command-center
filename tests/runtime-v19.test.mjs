...
test('Change Intelligence and premium layer consume the shared runtime',()=>{
  const change=read('change-intelligence-v18.js'),premium=read('premium-experience-v14.js');
  assert.match(change,/const runtime=window\.TitansRuntime/);
  assert.match(change,/runtime\.apiJson\('\/api\/data'/);
  assert.match(change,/runtime\.onAppRender/);
  assert.doesNotMatch(change,/new MutationObserver\(\(\)=>queueMicrotask\(watchView\)\)\.observe\(app/);
