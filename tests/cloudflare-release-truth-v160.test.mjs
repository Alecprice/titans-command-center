...
const retry=readFileSync(new URL('../scripts/lib/fetch-with-transport-retry.mjs',import.meta.url),'utf8');
...
test('release segmentation composes with the merged bounded headshot transport retry',()=>{
  assert.match(headshot,/import \{ fetchWithTransportRetry \} from '\.\/lib\/fetch-with-transport-retry\.mjs'/);
  assert.equal((headshot.match(/fetchWithTransportRetry\(/g)||[]).length,4);
  assert.match(retry,/const RETRYABLE_TRANSPORT_CODES = new Set\(\['ECONNRESET'\]\)/);
  assert.match(retry,/attempts = 2/);
  assert.match(retry,/if \(error\?\.name === 'AbortError' \|\| error\?\.name === 'TimeoutError'\) return false/);
  assert.match(retry,/if \(attempt >= attempts \|\| !isRetryableTransportError\(error\)\) throw error/);
