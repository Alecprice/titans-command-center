...
test('roster source verification preserves the real Sept 2 capture date',()=>{
  ...
  assert.match(freshness,/const ROSTER_VERIFIED_THROUGH='2026-09-06'/);
...
test('audited fallback keeps its Sept 2 audit date while exposing source verification',()=>{
  assert.match(freshness,/const verification=rosterVerification\(fresh\.roster\)/);
  assert.match(freshness,/Verified backup · \$\{verified\}/);
  assert.match(freshness,/Roster verified \$\{verified\}\$\{verifiedSuffix\}/);
  assert.match(freshness,/backup audit date remains \$\{verified\}/);
  assert.doesNotMatch(freshness,/const verification=fallback\?null:/);
});
