...
test('Ticket decision settle coordinator loads after the three decision surfaces',()=>{
  ...
  const settleIndex=fallback.indexOf("import './tickets-decision-settle-v149.js'");
  assert.ok(compare>=0&&finalists>compare&&signal>finalists&&settleIndex>signal);
});

test('same-tab shortlist reconciliation verifies compare keys and derived panels',()=>{
  assert.match(settle,/titans:tickets-shortlist-v123/);
...
test('settle repair is bounded to one frame and one storage wake-up without polling or network',()=>{
  assert.match(settle,/requestAnimationFrame/);
  assert.match(settle,/new StorageEvent\('storage'/);
  assert.doesNotMatch(settle,/setInterval|setTimeout|fetch\(|XMLHttpRequest|WebSocket|EventSource/);
