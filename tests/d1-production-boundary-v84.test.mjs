...
test('Worker D1 health is authoritative with a fresh bootstrap snapshot',async()=>{
  const snapshot={cache_key:'bootstrap:v1',payload:{ok:true,dataQuality:{contentAuditAt:'2026-09-08T00:00:00.000Z'}},source:'audited-fallback',fetched_at:new Date().toISOString(),expires_at:new Date(Date.now()+60_000).toISOString()};
  ...
  assert.equal(body.contentAudit,'2026-09-08T00:00:00.000Z');
});

test('Worker D1 health remains authoritative with no warehouse secret at all',async()=>{
  const snapshot={cache_key:'bootstrap:v1',payload:{ok:true,dataQuality:{contentAuditAt:'2026-09-08'}},source:'audited-fallback',fetched_at:new Date().toISOString(),expires_at:new Date(Date.now()+60_000).toISOString()};
