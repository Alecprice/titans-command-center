...
test('transition-sensitive mobile smokes wait for settled More-sheet geometry',()=>{
  ...
  assert.match(runtime,/def wait_mobile_sheet\(driver/);
  assert.match(runtime,/r\.bottom>dr\.top\+2/);
...
test('quality gate syntax-checks the live market smoke with current feature smokes',()=>{
  for(const path of [
...
test('Cloudflare deployment gates analytics on current mobile search account and market smokes',()=>{
