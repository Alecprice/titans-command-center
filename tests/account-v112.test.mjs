...
test('account preference migration is idempotent and stores only json preferences by auth user id',()=>{
  const sql=read('db/migrations/20260822_001_fan_user_preferences.sql');
  assert.match(sql,/create table if not exists fan_user_preferences/);assert.match(sql,/user_id text primary key/);assert.match(sql,/preferences jsonb not null default '\{\}'::jsonb/);assert.match(sql,/schema_version integer not null default 1/);assert.match(sql,/updated_at timestamptz not null default now\(\)/);assert.match(sql,/create index if not exists fan_user_preferences_updated_at_idx/);assert.match(sql,/never passwords, session tokens, or auth cookies/);
});
...
test('account browser smoke isolates onboarding and diagnoses each production stage',()=>{
  const smoke=read('scripts/account-browser-smoke.py');
  assert.match(smoke,/def prepare_returning_user\(driver\):/);assert.match(smoke,/titans:v10Onboarded/);assert.match(smoke,/def disable_sidebar_motion\(driver\):/);assert.match(smoke,/#sidebar\{transition:none!important;animation:none!important\}/);assert.match(smoke,/stage='wait-guest'/);assert.match(smoke,/stage='wait-mobile-shell'/);assert.match(smoke,/stage='wait-more'/);assert.match(smoke,/stage='wait-account-panel'/);assert.match(smoke,/stage='simulate-auth-outage'/);assert.match(smoke,/stage='wait-roster'/);assert.match(smoke,/runtimeVersion:window\.TitansRuntime\?\.version/);assert.match(smoke,/ariaHidden:sidebar\?\.getAttribute\('aria-hidden'\)/);assert.match(smoke,/result\['stage'\]=stage/);assert.match(smoke,/result\['state'\]=state\(d\)/);
});
