...
test('account browser smoke isolates onboarding and diagnoses each production stage',()=>{
  ...
  assert.match(smoke,/def prepare_returning_user\(driver\):/);assert.match(smoke,/titans:v10Onboarded/);assert.match(smoke,/def disable_sidebar_motion\(driver\):/);assert.match(smoke,/#sidebar\{transition:none!important;animation:none!important\}/);assert.match(smoke,/stage='wait-guest'/);assert.match(smoke,/stage='wait-more'/);assert.match(smoke,/stage='wait-account-panel'/);assert.match(smoke,/stage='simulate-auth-outage'/);assert.match(smoke,/stage='wait-roster'/);assert.match(smoke,/result\['stage'\]=stage/);assert.match(smoke,/result\['state'\]=state\(d\)/);
});
