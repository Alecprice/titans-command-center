...
test('mobile navigation polish is loaded and packaged offline',()=>{
  ...
  assert.match(sw,/titans-cc-brand-2026-v53/);
...
test('mobile sidebar becomes a bottom sheet above the dock',()=>{
  const css=read('mobile-navigation-v112.css');
  assert.match(css,/bottom:calc\(var\(--pwa-dock-h\) \+ 18px \+ env\(safe-area-inset-bottom\)\)/);
  assert.match(css,/max-height:min\(72dvh,660px\)/);
  assert.match(css,/\.sidebar\.open\{transform:translateY\(0\)!important/);
