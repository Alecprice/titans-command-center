...
test('near-live scheduler preserves an already-fresh bootstrap snapshot',async()=>{
  const database=new FakeD1();
  const env={TITANS_DB:database};
  const fetchedAt='2026-09-08T16:00:00.000Z';
  await putD1Snapshot(env,'bootstrap:v1',{ok:true,mode:'live-database',databaseAvailable:true,dataQuality:{contentAuditAt:'2026-09-08'},games:[{id:'keep-me'}]},{source:'verified-live-materialization',fetchedAt,ttlSeconds:86400});
