(() => {
  ...
  let context=null,contextKey='',running=false;
  ...
  async function loadContext(){
    ...
    running=true;try{
      const base={candidates:manualCandidates(),adds:new Map(),drops:new Map(),titans:[]};
      ...
      if(!s.sleeperUser||!s.leagueId){context=base;contextKey=key;return context}
      ...
      const seen=new Set();base.candidates=base.candidates.filter(p=>{const key=`${norm(p.name)}|${p.team}|${p.position}`;if(seen.has(key))return false;seen.add(key);return true}).slice(0,80);
      context=base;contextKey=key;return context;
    }catch{context={candidates:manualCandidates(),adds:new Map(),drops:new Map(),titans:[]};contextKey='';return context}finally{running=false}
  }
  ...
  async function mount(){
    ...
    const ctx=await loadContext();if(route()!==ROUTE||!host.isConnected)return;
  ...
  const observer=new MutationObserver(()=>{if(route()===ROUTE)queueMicrotask(mount)});if(app())observer.observe(app(),{childList:true,subtree:true});addEventListener('hashchange',()=>{contextKey='';if(route()===ROUTE)queueMicrotask(mount)});queueMicrotask(mount);
})();