(() => {
  ...
  function shareText(items,party){
    ...
    items.forEach((item,index)=>{
      ...
      lines.push(item.price!=null
        ?`Current start: ${money(item.price)} · ${party} ticket${party===1?'':'s'}: ${money(total)} before fees · ${sources}`
        :`${party} ticket${party===1?'':'s'}: current starting price unavailable before fees · ${sources}`);
  ...
  function settleReveal(center,key,frame=0,stableFrames=0){
    ...
    if(!center||!center.isConnected){
      if(frame<MAX_FOCUS_SETTLE_FRAMES)requestAnimationFrame(()=>settleReveal(center,key,frame+1,0));
      return;
    }
