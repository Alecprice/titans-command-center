(() => {
  ...
  function latestFinal(){return (state.base?.games||[]).filter(g=>/final/i.test(String(g.status||''))).map((game,index)=>({game,index,time:Date.parse(game.date)})).sort((a,b)=>{const at=Number.isFinite(a.time)?a.time:-Infinity,bt=Number.isFinite(b.time)?b.time:-Infinity;return bt-at||b.index-a.index})[0]?.game||null}
