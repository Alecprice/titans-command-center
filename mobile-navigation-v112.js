(() => {
  ...
  function pointerDown(event){const target=event.target instanceof Element?event.target:null;if(!phone.matches||!sidebar.classList.contains('open')||event.pointerType==='mouse'&&event.button!==0||target?.closest('a,button,input,select,textarea,label,[role="button"],[role="link"]'))return;drag={id:event.pointerId,y:event.clientY,last:event.clientY,started:performance.now()};sidebar.classList.add('dragging');try{sidebar.setPointerCapture(event.pointerId)}catch{}}
