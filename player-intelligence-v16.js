(() => {
  ...
  function latestStatus(data,fan){
    const p=data.player||{},name=p.name||'Player',injuries=arr(data.injuries),currentInjury=injuries[0];
    const depth=arr(fan?.depthChart?.changes).find(x=>String(x.playerId||'')===String(p.id||'')||slug(x.name)===slug(name));
    ...
    return {label:'Roster status',value:p.rosterStatus||'Active roster',copy:'No newer injury or depth-change row is loaded for this player.'};
  }
  ...
  function whatChanged(data,fan){
    const p=data.player||{},name=p.name||'',rows=[];
    ...
    const depth=arr(fan?.depthChart?.changes).find(x=>String(x.playerId||'')===String(p.id||'')||slug(x.name)===slug(name));if(depth)rows.push(['Depth chart',`${depth.type||'changed'} · ${depth.from??'—'} → ${depth.to??'—'}`,fan?.depthChart?.capturedAt?fmt(fan.depthChart.capturedAt):'Current snapshot']);
    const mentions=arr(fan?.playerStats).filter(x=>String(x.playerId||'')===String(p.id||'')).slice(0,2);if(mentions.length)rows.push(['Production',`${mentions.length} recent player-stat row${mentions.length===1?'':'s'} loaded`,'Game data']);
  ...
  function newsAndMoves(data,fan){
    const p=data.player||{},name=slug(p.name),rows=[];
    for(const item of arr(fan?.playerStats).filter(x=>String(x.playerId||'')===String(p.id||'')).slice(0,4))rows.push({kind:'Game data',title:`${item.statGroup||'Stats'} · Week ${item.week??'—'}`,meta:fmt(item.kickoff)});
    ...
    const pageData=window.__titansBootstrap||null;
    if(pageData)for(const tx of arr(pageData.transactions).filter(x=>slug(x.description).includes(name)).slice(0,4))rows.push({kind:'Transaction',title:tx.description,meta:fmt(tx.date)});
    return rows.length?`<div class="v16-move-list">${rows.slice(0,8).map(x=>`<article><small>${esc(x.kind)}</small><strong>${esc(x.title)}</strong><span>${esc(x.meta)}</span></article>`).join('')}</div>`:'<div class="v16-empty"><strong>No additional timeline rows loaded.</strong><span>The page will expand automatically as transactions, injuries and game stats accumulate.</span></div>';
  }

  function contractCard(data,fan){
    const p=data.player||{},contract=arr(fan?.contracts).find(x=>String(x.playerId||'')===String(p.id||'')||slug(x.name)===slug(p.name));
    ...
    const money=v=>number(v)==null?'—':new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}).format(Number(v));
  ...
  function renderLayer(data,fan){
    const root=document.querySelector('.player-profile-rich');if(!root||root.querySelector('.v16-player-intel'))return;
    const p=data.player||{},rows=arr(data.stats),status=latestStatus(data,fan),last=lastGame(rows),quick=metricSummary(last),changes=whatChanged(data,fan),profile=getJson(PROFILE_KEY,{}),favorite=profile.favorite===p.name;
    const layer=document.createElement('section');layer.className='v16-player-intel';layer.innerHTML=`
      <section class="v16-player-command">
        <header><div><small>PLAYER COMMAND CENTER</small><h2>${esc(p.name||'Titans player')}</h2><p>Quick answer first. Game logs, trends and deeper football context stay one tap away.</p></div><button type="button" data-v16-favorite aria-pressed="${favorite}">${favorite?'★ Favorite':'☆ Make favorite'}</button></header>
        <div class="v16-quick-grid"><article><small>${esc(status.label)}</small><strong>${esc(status.value)}</strong><span>${esc(status.copy)}</span></article><article><small>Last loaded game</small><strong>${esc(last?`Week ${last.week??'—'}`:'Awaiting stats')}</strong><span>${esc(last?fmt(last.kickoff):'No game-stat row loaded')}</span></article>${quick.slice(0,2).map(x=>`<article><small>${esc(x.label)}</small><strong>${esc(x.value)}</strong><span>Latest loaded game</span></article>`).join('')}</div>
      </section>
      <section class="v16-player-changed"><header><div><small>WHAT CHANGED?</small><h3>Latest player movement</h3></div><span>Verified loaded data only</span></header>${changes.map(x=>`<article><small>${esc(x[0])}</small><strong>${esc(x[1])}</strong><span>${esc(x[2])}</span></article>`).join('')}</section>
      ...
      <section class="v16-player-pane" data-v16-pane="career" hidden><div class="v16-grid two"><article class="v16-panel"><small>CAREER COVERAGE</small><h3>${esc(p.experience?`${p.experience} year${String(p.experience)==='1'?'':'s'} experience`:'Experience not loaded')}</h3><p>${esc(p.college?`${p.college} · `:'')}The game log can include any seasons currently backfilled for this player. A complete career archive is not claimed until historical ingestion is complete.</p></article><article class="v16-panel"><small>CONTRACT</small><h3>Current loaded contract context</h3>${contractCard(data,fan)}</article></div></section>
      <section class="v16-player-pane" data-v16-pane="timeline" hidden><header><div><small>PLAYER TIMELINE</small><h3>Game, injury and roster evidence</h3></div><a href="#command">Open full knowledge graph →</a></header>${newsAndMoves(data,fan)}</section>`;
    root.append(layer);
    layer.addEventListener('click',event=>{
      ...
      const fav=event.target.closest?.('[data-v16-favorite]');if(fav){const next=getJson(PROFILE_KEY,{});next.favorite=favorite?'':p.name;setJson(PROFILE_KEY,next);fav.textContent=favorite?'☆ Make favorite':'★ Favorite';fav.setAttribute('aria-pressed',String(!favorite))}
    });
  }
  ...
  async function enhance(){
    ...
    try{const [profile,fan]=await Promise.all([fetch(`/api/player?id=${encodeURIComponent(id)}`,{cache:'no-store'}).then(r=>r.ok?r.json():null),fanIntel()]);if(current!==serial||route()!=='player'||playerId()!==id||!profile?.ok)return;renderLayer(profile,fan?.ok?fan:{})}catch{}
  }
