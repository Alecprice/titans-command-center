(() => {
  ...
  async function connectSleeper(username=state.sleeperUser){
    const clean=String(username||'').trim();if(!clean){connectSerial++;sleeper.connecting=false;sleeper.error='Enter a Sleeper username.';render();return;}
    if(sleeper.connecting)return;
    const serial=++connectSerial;sleeper.connecting=true;sleeper.error='';
    try{
      ...
      leagueLoadSerial++;sleeper.loading=false;clearLeagueData();
