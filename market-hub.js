...
const DEFAULT_VISIBLE_ROWS=72,ROW_PAGE_SIZE=72;
const marketUi={showAlt:false,book:'all',category:'all',event:'all',visibleRows:DEFAULT_VISIBLE_ROWS};
...
function marketGroups(rows,eventMap){const order=['Spread','Moneyline','Total'];const groups=new Map();for(const r of rows||[]){const key=r.marketName||'Other';if(!groups.has(key))groups.set(key,[]);groups.get(key).push(r)}return [...groups].sort((a,b)=>{const ai=order.indexOf(a[0]),bi=order.indexOf(b[0]);return (ai<0?99:ai)-(bi<0?99:bi)||a[0].localeCompare(b[0])}).map(([name,items])=>{const sorted=sortedMarketRows(items,eventMap);return `<section class="mh-market-group"><header><strong>${mhEsc(name)}</strong><span>${sorted.length} visible line${sorted.length===1?'':'s'}</span></header><div>${sorted.map(item=>row(item,eventMap)).join('')}</div></section>`}).join('')}
...
function resetVisibleRows(){marketUi.visibleRows=DEFAULT_VISIBLE_ROWS}
...
function bindControls(app,d,hub){
  ...
  hub.querySelector('#mh-event-filter')?.addEventListener('change',event=>{marketUi.event=event.target.value;resetVisibleRows();renderMarket(app,d)});
  hub.querySelector('#mh-book-filter')?.addEventListener('change',event=>{marketUi.book=event.target.value;resetVisibleRows();renderMarket(app,d)});
  hub.querySelector('#mh-category-filter')?.addEventListener('change',event=>{marketUi.category=event.target.value;resetVisibleRows();renderMarket(app,d)});
  hub.querySelector('#mh-alt-toggle')?.addEventListener('click',()=>{marketUi.showAlt=!marketUi.showAlt;resetVisibleRows();renderMarket(app,d)});
  hub.querySelector('#mh-show-more')?.addEventListener('click',()=>{marketUi.visibleRows+=ROW_PAGE_SIZE;renderMarket(app,d)});
  hub.querySelector('#mh-show-all')?.addEventListener('click',()=>{marketUi.visibleRows=Number.MAX_SAFE_INTEGER;renderMarket(app,d)});
}
function renderMarket(app,d){
  normalizeSelection(d);
  const existingHead=app.querySelector('.page-head'),eventMap=new Map((d.events||[]).map(e=>[eventId(e),e])),shown=filteredRows(d),visible=shown.slice(0,marketUi.visibleRows),remaining=Math.max(0,shown.length-visible.length),[qualityLabel,qualityTone]=quality(d),isRef=/reference/.test(d.sourceMode||''),title=isRef?'Published Titans line reference':d.quality==='unavailable'?'Titans market status':'Live Titans market board';
  ...
  const densityMarkup=remaining?`<div class="mh-density" role="status"><span>Showing <b>${visible.length}</b> of <b>${shown.length}</b> matching rows. Filters are the fastest way to narrow a large board.</span><div><button type="button" class="button secondary" id="mh-show-more">Show ${Math.min(ROW_PAGE_SIZE,remaining)} more</button><button type="button" class="button secondary" id="mh-show-all">Show all ${shown.length}</button></div></div>`:'';
  hub.innerHTML=`<div class="mh-head"><div><small>Odds & props</small><h2>${title}</h2><p>${mhEsc(d.message||'Market data refresh completed.')}</p></div><button id="mh-refresh" type="button" class="button primary">Refresh board</button></div><div class="mh-status"><span><b>${mhEsc(d.provider||'No source')}</b> source</span><span><b>${(d.odds||[]).length}</b> market rows</span><span><b>${d.propsAvailable?'Available':'Not supplied'}</b> player props</span><span class="${qualityTone}"><b>${mhEsc(qualityLabel)}</b> freshness</span><span><b>${mhEsc(age(d.fetchedAt))}</b> checked</span>${d.referencePublishedAt?`<span><b>${mhEsc(dt(d.referencePublishedAt))}</b> published</span>`:''}</div>${eventMarkup?`<div class="mh-events">${eventMarkup}</div>`:''}${d.referenceNotice?`<div class="mh-reference-notice"><strong>Published reference — not live odds.</strong><span>${mhEsc(d.referenceNotice)}</span>${d.referenceExpiresAt?`<em>Hidden automatically at kickoff: ${mhEsc(dt(d.referenceExpiresAt))}</em>`:''}${links(d.sources)}</div>`:''}${controls(d,shown)}${densityMarkup}<div class="mh-table">${marketGroups(visible,eventMap)||'<div class="mh-empty">No market rows match these filters. Try another game or sportsbook, show alternate lines, or switch the market type.</div>'}</div>${(d.diagnostics||[]).length?`<details class="mh-diagnostics"><summary>Technical details</summary><p>${d.diagnostics.map(mhEsc).join(' · ')}</p></details>`:''}<div class="mh-note">Use the freshness and source labels above to judge each line. Provider checks are quota-aware and may reuse a recent server response. Alternate lines are hidden by default to keep the board readable. Player props are shown only when a real source supplies them. This section is informational only.</div>`;
...
addEventListener('hashchange',()=>{marketRequestSerial++;marketUi.showAlt=false;marketUi.book='all';marketUi.category='all';marketUi.event='all';resetVisibleRows();const app=document.querySelector('#app');if(app){delete app.dataset.marketHub;app.removeAttribute('aria-busy')}queueMicrotask(()=>loadMarket(false))});
