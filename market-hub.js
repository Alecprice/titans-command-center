...
const categoryLabel=value=>value==='player_prop'?'Player props':value==='game_line'?'Game lines':'All markets';
let marketRequestSerial=0;
let marketDataCache=null;
const marketUi={showAlt:false,book:'all',category:'all'};
...
function row(r,eventMap){
  ...
  const event=eventMap.get(String(r.providerEventId||r.eventId||''));
...
function marketGroups(rows,eventMap){const order=['Spread','Moneyline','Total'];const groups=new Map();for(const r of rows||[]){const key=r.marketName||'Other';if(!groups.has(key))groups.set(key,[]);groups.get(key).push(r)}return [...groups].sort((a,b)=>{const ai=order.indexOf(a[0]),bi=order.indexOf(b[0]);return (ai<0?99:ai)-(bi<0?99:bi)||a[0].localeCompare(b[0])}).map(([name,items])=>`<section class="mh-market-group"><header><strong>${mhEsc(name)}</strong><span>${items.length} line${items.length===1?'':'s'}</span></header><div>${items.map(item=>row(item,eventMap)).join('')}</div></section>`).join('')}
...
function filteredRows(d){return (d.odds||[]).filter(r=>(marketUi.showAlt||!r.alt)&&(marketUi.book==='all'||bookKey(r)===marketUi.book)&&(marketUi.category==='all'||r.category===marketUi.category))}
function controls(d,shown){
  const rows=d.odds||[],books=availableBooks(rows),altCount=rows.filter(r=>r.alt).length,hasProps=rows.some(r=>r.category==='player_prop');
...
function renderMarket(app,d){
  ...
  hub.innerHTML=`<div class="mh-head"><div><small>Odds & props</small><h2>${title}</h2><p>${mhEsc(d.message||'Market data refresh completed.')}</p></div><button id="mh-refresh" type="button" class="button primary">Refresh odds</button></div><div class="mh-status"><span><b>${mhEsc(d.provider||'No source')}</b> source</span><span><b>${(d.odds||[]).length}</b> market rows</span><span><b>${d.propsAvailable?'Available':'Not supplied'}</b> player props</span><span class="${qualityTone}"><b>${mhEsc(qualityLabel)}</b> freshness</span><span><b>${mhEsc(age(d.fetchedAt))}</b> checked</span>${d.referencePublishedAt?`<span><b>${mhEsc(dt(d.referencePublishedAt))}</b> published</span>`:''}</div>${eventMarkup?`<div class="mh-events">${eventMarkup}</div>`:''}${d.referenceNotice?`<div class="mh-reference-notice"><strong>Published reference — not live odds.</strong><span>${mhEsc(d.referenceNotice)}</span>${d.referenceExpiresAt?`<em>Hidden automatically at kickoff: ${mhEsc(dt(d.referenceExpiresAt))}</em>`:''}${links(d.sources)}</div>`:''}${controls(d,shown)}<div class="mh-table">${marketGroups(shown,eventMap)||'<div class="mh-empty">No market rows match these filters. Try another sportsbook, show alternate lines, or switch the market type.</div>'}</div>${(d.diagnostics||[]).length?`<details class="mh-diagnostics"><summary>Technical details</summary><p>${d.diagnostics.map(mhEsc).join(' · ')}</p></details>`:''}<div class="mh-note">Use the freshness and source labels above to judge each line. Alternate lines are hidden by default to keep the board readable. Player props are shown only when a real source supplies them. This section is informational only.</div>`;
...
async function loadMarket(force=false){
  ...
  try{
    ...
    marketDataCache=d;renderMarket(app,d);
  }catch(e){
    if(requestId!==marketRequestSerial||mhRoute()!=='markets')return;
...
addEventListener('hashchange',()=>{marketRequestSerial++;marketDataCache=null;marketUi.showAlt=false;marketUi.book='all';marketUi.category='all';const app=document.querySelector('#app');if(app){delete app.dataset.marketHub;app.removeAttribute('aria-busy')}queueMicrotask(()=>loadMarket(false))});
