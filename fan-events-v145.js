(() => {
  ...
  const VERSION='146.0.0';
  const API='/api/fan-events';
  const CENTRAL='America/Chicago';
  const SKIDDLE_LOGO='https://d1plawd8huk6hh.cloudfront.net/assets/logo/png/skiddle-logo-white-landscape.png';
  ...
  function sourceLinks(event){
    ...
    const links=sources.map(source=>{
      ...
      if(!url)return'';
      if(source?.provider==='Skiddle'){
        return `<a class="fan-event-skiddle-source" href="${esc(url)}" target="_blank" rel="noopener noreferrer" data-fan-events-source="Skiddle" aria-label="View this event on Skiddle"><span class="fan-event-skiddle-logo-space"><img src="${SKIDDLE_LOGO}" alt="Skiddle" width="96" height="40" loading="lazy" decoding="async"></span><span>View on Skiddle ↗</span></a>`;
      }
  ...
  function content(payload){
    ...
    const providers=`<details class="fan-events-sources"><summary>Provider scope & availability</summary><div class="fan-events-provider-grid">${providerSummary(payload)}</div><p>Eventbrite is limited to events owned by organizations authorized for the connected token. Skiddle uses a bounded geographic search; every Skiddle listing is credited with the official Skiddle brand logo and links directly to the Skiddle event URL returned by its API.</p></details>`;
    const cards=events.length?`<div class="fan-events-rail" aria-label="Upcoming Nashville events">${events.map(eventCard).join('')}</div>`:`<div class="fan-events-empty"><strong>No upcoming events returned in the current window.</strong><p>This can mean a provider has no matching inventory for Nashville or a connected source is temporarily unavailable.</p></div>`;
