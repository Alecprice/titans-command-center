(() => {
  ...
  document.addEventListener('click',event=>{const button=event.target.closest?.('[data-custom-remove]');if(!button)return;const links=readLinks();links.splice(Number(button.dataset.customRemove),1);writeLinks(links);render()});
