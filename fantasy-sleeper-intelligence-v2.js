(() => {
  ...
  let serial=0,lastKey='',lastWeek=0,lastPayload=null,lastError='',running=false,pending=false;
  ...
  const connectionKey=s=>`${String(s?.username||'').trim().toLowerCase()}|${String(s?.leagueId||'')}`;
  ...
  function renderIntel(host,{league,user,rosters,players,adds,drops}){
    host.querySelector('.fsi-error')?.remove();host.querySelector('.fsi-wrap')?.remove();
  ...
  function renderError(host,message){
    host.querySelector('.fsi-wrap')?.remove();host.querySelector('.fsi-error')?.remove();
    const box=document.createElement('div');box.className='fsi-error';box.textContent=message;host.append(box);
  }
  async function hydrate(){
    if(route()!==ROUTE)return;
    const s=state(),content=root()?.querySelector('.fantasy-content');if(!content||!s.username||!/^\d{6,32}$/.test(s.leagueId))return;
    const visibleSleeper=Boolean(content.querySelector('#sleeper-connect'));if(!visibleSleeper){content.querySelector('.fsi-wrap')?.remove();content.querySelector('.fsi-error')?.remove();return}
    const key=connectionKey(s);
    if(running){pending=true;return}
    if(key===lastKey&&content.querySelector('.fsi-wrap'))return;
    if(key===lastKey&&lastError&&content.querySelector('.fsi-error'))return;
    if(key===lastKey&&s.week!==lastWeek&&lastPayload){lastWeek=s.week;style();renderIntel(content,lastPayload);return}
    if(key===lastKey&&s.week!==lastWeek&&lastError){lastWeek=s.week;renderError(content,lastError);return}
    lastKey=key;lastWeek=s.week;lastPayload=null;lastError='';running=true;const mine=++serial;
    try{
      style();
      ...
      const live=state(),liveContent=root()?.querySelector('.fantasy-content');
      if(mine!==serial||route()!==ROUTE||connectionKey(live)!==key)return;
      const payload={league,user,rosters,players,adds,drops};lastPayload=payload;lastError='';lastWeek=live.week;
      if(liveContent?.querySelector('#sleeper-connect'))renderIntel(liveContent,payload);
    }catch(error){
      const live=state(),liveContent=root()?.querySelector('.fantasy-content');
      if(mine!==serial||route()!==ROUTE||connectionKey(live)!==key)return;
      lastPayload=null;lastWeek=live.week;lastError=error?.name==='AbortError'?'Sleeper intelligence timed out. Try again from Sleeper Connect.':`Sleeper intelligence unavailable: ${error?.message||error}`;
      if(liveContent?.querySelector('#sleeper-connect'))renderError(liveContent,lastError);
    }finally{
      running=false;
      const live=state();
      if(pending||(route()===ROUTE&&connectionKey(live)!==key)){pending=false;queueMicrotask(hydrate)}
    }
  }
  const observer=new MutationObserver(()=>queueMicrotask(hydrate));if(root())observer.observe(root(),{childList:true,subtree:true});
  addEventListener('hashchange',()=>{serial++;lastKey='';lastWeek=0;lastPayload=null;lastError='';pending=false;queueMicrotask(hydrate)});queueMicrotask(hydrate);
})();
